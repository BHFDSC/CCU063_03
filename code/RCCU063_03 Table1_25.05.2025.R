...
table1 <- table1(~ agefinal + eth5 + imd_quintile + prev_preg + region + folic_acid_pre_pregnancy + ovsvischcat + complexsocialfactors + booking_after_10weeks|interpreter_use , data = mydata1)
print(table1)
...
table1<- table1(~ agefinal + eth5 + imd_quintile + prev_preg + region + folic_acid_pre_pregnancy + ovsvischcat + complexsocialfactors + booking_after_10weeks + fact_of_gp_interaction + fact_of_ae_interaction|interpreter_use , 
                data = mydata1, render.categorical=my.render.cat)
print(table1)
...
#Save table
write.csv(table, "/CCU063_03/table1.25.05.csv", row.names = FALSE)
...
#Save table
write.csv(final_summary, "/CCU063_03/medianagetotal.24.05.csv", row.names = FALSE)
